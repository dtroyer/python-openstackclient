...
"""Identity v2 API Library Tests"""

from requests_mock.contrib import fixture

from keystoneclient import session
from openstackclient.api import identity_v2
...
class TestIdentityAPIv2(utils.TestCase):

    def setUp(self):
        super(TestIdentityAPIv2, self).setUp()
        sess = session.Session()
        self.api = identity_v2.APIv2(session=sess, endpoint=FAKE_URL)
        self.requests_mock = self.useFixture(fixture.Fixture())


class TestProject(TestIdentityAPIv2):
    ...
    def test_project_list_no_options(self):
        self.requests_mock.register_uri(
            'GET',
            FAKE_URL + '/tenants',
            json={'tenants': self.LIST_TENANT_RESP},
            status_code=200,
        )
